...
public:
	...
	Common::Point getMousePos() const override;
	void clearRect(const Common::Rect &r) const override;
	static bool validateMode(Common::Platform p) { return checkDriver(&_driverFile, 1); }
private:
	void setupRenderProc() override;
...
protected:
	...
	typedef void (*OnTopRenderProc)(byte*, int, const byte*, int, int, int, int, int);
	OnTopRenderProc _renderOnTop;
