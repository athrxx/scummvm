...
void ScummEngine::saveLoadWithSerializer(Common::Serializer &s) {
	...
	if (s.isLoading() && _game.version < 7 && _game.heversion == 0) {
	...
	// Unfortunately, it is not totally easy to write the sound engine data size before that data, because we
	// cannot seek around in the compressed write stream. So, we write to a temp stream.
